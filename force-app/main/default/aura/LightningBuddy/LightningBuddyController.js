({
    "handleBuddyEvent": function (cmp, event, helper) {
        ...
        cmp.set('v.customCriteriaEvaluated', event.getParam('customCriteriaEvaluated'));
            ...
            if (cmp.get('v.pageType') === 'Home' && tabsAndEvents[evtKey] == null) {
            	tabsAndEvents[evtKey] = currentTime;
            }
            else if (cmp.get('v.pageType') === 'Home' && tabsAndEvents[evtKey] != null && (currentTime - tabsAndEvents[evtKey] <= 3500)) {
        ...
        if (!userInfo) {
            ...
            usrPromise.then(
                $A.getCallback(function (result) {
                    ...
                    console.log('userInfo' + JSON.stringify(cmp.get('v.userInfo')));
                    ...
                    console.log('getAction' + JSON.stringify(getAction))
                    console.log('recordId' + cmp.get('v.recordId'))
            ...
            console.log('recordIdElseBlock' + cmp.get('v.recordId'))
            var actionPromise = helper.executeAction(cmp, getAction);
            actionPromise.then(
                $A.getCallback(function (result) {
                    helper.afterUserInfoCallback(result, cmp, event);
