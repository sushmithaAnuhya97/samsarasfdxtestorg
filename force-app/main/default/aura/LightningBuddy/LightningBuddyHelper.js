({
            ...
            for (var i = 0; i < actions.length; i++) {
                ...
                if (actionToShow) {
                    if (this.isEnabledForProfile(usrInfo, actionToShow)) {
                        if (!actionToShow.Use_Custom_Criteria_Handler__c && this.meetsRecordCriteria(actionToShow, cmp)) {
                            this.handleAction(cmp, event, actionToShow);
                            break;
                        } else if (actionToShow.Use_Custom_Criteria_Handler__c) {
    ...
    "handleAction": function (cmp, event, action) {
        ...
        var showOnlyOnce = (action.Recurrence__c == 'Only once' ? true : false);
        if (action && action.RecordType.Name === 'Rich Text') {
            $A.createComponents([
                ["c:ModalContent", { "modalContent": action.Action_Text__c }],
            ...
            $A.get('e.lightning:openFiles').fire({
                recordIds: [action.File_Id__c]
            });

        } else if (action && action.RecordType.Name === 'Popover') {
            cmp.set('v.popoverMsgToSend', action.Popover_Text__c);
        } else if (action && action.RecordType.Name === 'Image Carousel') {
            $A.createComponents([
                ["c:ImageCarousel", { "cardInfo": action.Image_Carousel_Config__c }],
                ["c:ModalFooter", { "actionId": action.Id, "showOnlyOnce": showOnlyOnce }]
            ],
                function (components, status) {
                    if (status === "SUCCESS") {
            ...
            $A.createComponents([["c:Video",
                {
                    "videoSrc": action.Video_Link__c,
                    "videoType": action.Video_Type__c
                }
            ],
            ["c:ModalFooter", { "actionId": action.Id, "showOnlyOnce": showOnlyOnce }]
            ],
            ...
            var args = {
                actionName: cmp.get('v.object') + '.' + action.Quick_Action_API_Name__c,
            ...
            var modalBody;
            var cmpParams = {
                "flowName": action.Lightning_Flow_API_Name__c,
                "actionId": action.Id,
                "showOnlyOnce": showOnlyOnce
            };
            if (action.Record_Id_as_Input_Variable__c) {
            ...
            var eventParams = {
                "title": action.Welcome_Mat_Title__c,
                "bodyText": action.Welcome_Mat_Body__c,
                "backgroundImgUrl": action.Welcome_Mat_Background_Image__c,
                "tileConfigJson": action.Welcome_Mat_Tile_Config__c,
                "actionId": action.Id,
                "showOnlyOnce": showOnlyOnce
            };
            appEvent.setParams(eventParams);
            ...
            var alertParams = {
                mode: action.Toaster_Alert_Mode__c,
                key: action.Toaster_Alert_Icon__c,
                type: action.Toaster_Alert_Type__c,
                title: action.Toaster_Alert_Title__c,
                message: action.Toaster_Alert_Message__c,
                duration: action.Toaster_Alert_Duration__c * 1000
            };
            if (action.Toaster_Alert_Param_Config__c) {
                var alertMsgParams = JSON.parse(action.Toaster_Alert_Param_Config__c);
                if (alertMsgParams && alertMsgParams.length > 0) {
                    alertParams.messageTemplate = action.Toaster_Alert_Message__c;
    ...
    "isEnabledForProfile": function (usrInfo, actionToShow) {
        ...
        if (!actionToShow.Enabled_Profiles__c) {
            return true;
        } else {
            if (actionToShow.Enabled_Profiles__c.includes(usrProfileId)) {
                return true;
            } else if (actionToShow.Enabled_Profiles__c.includes(usrProfileName)) {
                return true;
            } else {
                return false;
            }
    ...
    "meetsRecordCriteria": function (actionToShow, cmp) {
        ...
        if (actionToShow.Record_Update_Criteria__c) {
            var criteriaList = JSON.parse(actionToShow.Record_Update_Criteria__c);
